...
public class NonDamagingEnemyController : MonoBehaviour
{
    ...
    void Update()
    {
        if (Vector2.Distance(transform.position, targetPlayer.healthController.Position) > 0) // If the distance between the enemy and player is more than 0 move towards the player 
        {
            transform.position = Vector2.MoveTowards(transform.position, targetPlayer.healthController.Position, speed * Time.deltaTime); // by using enemy's and player's position and speed to calculate
        }

        if (Vector2.Distance(transform.position, targetPlayer.healthController.Position) < 1)
        {
            if (!attackDelay)
            {
                targetPlayer.healthController.SpeedDecrease();
